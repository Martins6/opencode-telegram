...
2026-08-22-21-57 | Added OpenCode GitHub workflow integration - two `.github/workflows/opencode*.yml` files wire the `build` agent into issues/PRs (slash-command trigger on `/opencode` or `/oc`) and trigger automatic code review on PR open and every new commit, using the OpenCode GitHub App + OIDC and `minimax-coding-plan/MiniMax-M3`. README "GitHub Workflow" subsection added.
